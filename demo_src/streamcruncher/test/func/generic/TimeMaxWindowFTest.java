...
import streamcruncher.test.func.TimeMaxWindowTest;

/*
 * Author: Ashwin Jayaprakash Date: Sep 10, 2006 Time: 4:49:51 PM
 */

public abstract class TimeMaxWindowFTest extends TimeMaxWindowTest {
    @Override
    ...
    protected String getRQL() {
        return "select event_id, vehicle_id, speed, " + getCurrentTimestampKeyword()
                + " from test (partition store last " + getWindowSizeSeconds() + " seconds max "
                + getWindowSizeSeconds() + ") as testStr where testStr.$row_status is not dead;";
    }
}
