 ...
 */
public abstract class SlidingWF2PartitionTest extends OrderGeneratorTest {
    ...
    protected String getRQL() {
        ...
        return "select "
                + csv
                + " from test (partition by country, item_sku store last "
                + windowSize
                + ") as testStr where testStr.$row_status is not dead order by country, item_sku, order_id;";
    }
    ...
    protected void verify(List<BatchResult> results) {
        ...
        for (BatchResult result : results) {
            ...
            for (Object[] objects : rows) {
                 ...
                 * "order_time", "order_id"
                 */
                Map<String, SortedSet<Number>> countryData = partitionedData.get(objects[0]);
                if (countryData == null) {
