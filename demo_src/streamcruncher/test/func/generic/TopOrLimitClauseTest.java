...
/**
 * This Test demonstrates the "TOP | FIRST xx" clause or the equivalent "LIMIT
 * xx [OFFSET yy]" clause.
 */
public abstract class TopOrLimitClauseTest extends OrderGeneratorTest {
    ...
    protected Iterator<Object[]> getData() {
        Iterator<Object[]> iter = new Iterator<Object[]>() {
            ...
            public Object[] next() {
                ...
                event[5] = new Timestamp(TopOrLimitClauseTest.this.getEventTimeStamp(counter));
    ...
    protected String getRQL() {
        String columns = "country, state, city, item_sku, item_qty, order_time, order_id";

        return "select " + getTopClause() + " " + columns
                + " from test (partition by country, state, city store latest 10) as testStr"
                + " where testStr.$row_status is new " + getLimitClause() + ";";
    }

    /**
     * @return The clause to retrieve the first Row.
     */
    protected String getTopClause() {
        return "";
    }
    ...
    protected void verify(List<BatchResult> results) {
        ...
        for (BatchResult result : results) {
            ...
            Assert.assertEquals(rows.size(), 1, "Batch contains more than 1 Event");
        }
    }
}