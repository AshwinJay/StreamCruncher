...
import streamcruncher.innards.impl.artifact.GeneratorSpec;
...
public class FirebirdParser extends AbstractParser {
    public FirebirdParser(ParserParameters parserParameters) throws QueryParseException {
        super(parserParameters);
    }

    @Override
    protected TableSpec customizeResultTableSpec(RowSpec rowSpec, IndexSpec[] indexSpecs) {
        MiscSpec[] others = { new GeneratorSpec(resultTable.getSchema(), resultTable.getName()
                + "_gen", resultTable.getFQN(), null) };
    ...
    protected IndexSpec createIndexSpec(String schema, String name, String tableFQN,
        ...
        return new FirebirdIndexSpec(schema, name, tableFQN, unique, columnName, ascending);
    }

    @Override
    protected IndexSpec createIndexSpec(String schema, String name, String tableFQN,
            boolean unique, String[] columnNames, boolean[] ascending) {
        return new FirebirdIndexSpec(schema, name, tableFQN, unique, columnNames, ascending);
    }
    ...
    protected String getFirstIdColumnInResultSQL() {
        return "GEN_ID(" + resultTable.getFQN() + "_gen, 1)";
    }
